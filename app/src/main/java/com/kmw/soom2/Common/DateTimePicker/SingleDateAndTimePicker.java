...
public class SingleDateAndTimePicker extends LinearLayout {
    ...
    private List<OnCustomClick> customListeners = new ArrayList<>();
    private List<OnCancelClick> cancelClicks = new ArrayList<>();
...
//        }, DELAY_BEFORE_CHECK_PAST);
    ...
    public void clickDateChange(OnCustomClick listener){
        this.customListeners.add(listener);
    }

    public void clickCancelDialog(OnCancelClick listener){
    ...
    public Date getDate() {
        ...
        if (displayDays) {
            ...
            if (displayYears) {
                Log.i("TEST2","test : " + yearsPicker.getCurrentYear());
                calendar.set(Calendar.YEAR, yearsPicker.getCurrentYear());
            }
        ...
        Log.i("picker","month : " + new SimpleDateFormat("yyyy-MM-dd").format(calendar.getTime()));
    ...
    private void onClickTest(){
        ...
        final CharSequence format = isAmPm ? FORMAT_12_HOUR : FORMAT_24_HOUR;
        final String displayed = DateFormat.format(format, date).toString();
        Log.i("TEST","test : " + simpleDateFormat.format(date) + ", display : " + displayed);
        for (OnCustomClick listener : customListeners) {
            Log.d("??????","for");
            listener.onDateChanged(displayed, date);
        }
    }
    ...
    private void onCancelTest(){
        for (OnCancelClick listener : cancelClicks){
            Log.d("??????"," cancel");
